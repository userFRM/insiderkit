...
//!
//! ```text
//! $INSIDERKIT_CACHE_DIR/          (default: XDG cache / insiderkit)
//! ├── insider-2024.parquet        ← cached body
//! ├── insider-2024.parquet.etag
//! ├── insider-2023.parquet
//! └── insider-2023.parquet.etag
//! ```
//!
//! # Fetch flow (per call)
//!
...
impl CachedFetcher {
    pub fn new(http: reqwest::Client, base_url: String, cache_dir: PathBuf) -> Self {
        let mirror_url = Some(
            std::env::var("INSIDERKIT_MIRROR_URL")
    ...
    /// Fetch a parquet file by logical key (e.g. `"insider-2024"`).
    ///
...
///
/// Priority:
/// 1. `$INSIDERKIT_CACHE_DIR` env var.
/// 2. XDG/platform cache dir for the `insiderkit` application.
/// 3. Fallback: `~/.cache/insiderkit`.
pub(crate) fn default_cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("INSIDERKIT_CACHE_DIR") {
        return PathBuf::from(dir);
    }
    if let Some(proj) = directories::ProjectDirs::from("", "", "insiderkit") {
...
fn dirs_fallback() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        std::env::var("LOCALAPPDATA")
            .map(|d| PathBuf::from(d).join("insiderkit").join("cache"))
            .unwrap_or_else(|_| PathBuf::from("insiderkit-cache"))
    }
    #[cfg(not(target_os = "windows"))]
    {
        std::env::var("HOME")
            .map(|h| PathBuf::from(h).join(".cache").join("insiderkit"))
            .unwrap_or_else(|_| PathBuf::from(".insiderkit-cache"))
    }
}
...
mod tests {
    ...
    async fn etag_304_returns_cached() {
        ...
        let body = b"insider-parquet-bytes";
        ...
        let first = fetcher.fetch("insider-2024").await.unwrap();
        assert_eq!(first.as_ref(), body);
        let second = fetcher.fetch("insider-2024").await.unwrap();
    ...
    async fn stale_cache_fallback() {
        ...
        let server = MockServer::start().await;
        let stale_body = b"stale-insider-data";
        ...
        let http = reqwest::Client::builder()
            ...
            .unwrap();
        let cache_dir = tempfile::TempDir::new().unwrap();
        let cache_file = cache_dir.path().join("insider-2024.parquet");
        tokio::fs::write(&cache_file, stale_body).await.unwrap();

        let mut fetcher = CachedFetcher::new(http, server.uri(), cache_dir.path().to_path_buf());
        fetcher.set_mirror_url(None);

        let result = fetcher.fetch("insider-2024").await.unwrap();
    ...
    async fn corrupt_stale_cache_rejected_when_manifest_present() {
        ...
        let manifest_json = format!(r#"{{"insider-2024.parquet":"sha256:{correct_digest}"}}"#);
        ...
        let http = reqwest::Client::builder()
            ...
            .unwrap();
        let cache_dir = tempfile::TempDir::new().unwrap();
        tokio::fs::write(cache_dir.path().join("insider-2024.parquet"), b"CORRUPT")
            .await
            .unwrap();
        ...
        let err = fetcher.fetch("insider-2024").await.unwrap_err().to_string();
        assert!(
