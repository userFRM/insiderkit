//! Unified error type for insiderkit. All public methods return `insiderkit::Result<T>`.
use thiserror::Error;
...
mod tests {
    ...
    fn checksum_mismatch_displays_both_digests() {
        let e = Error::ChecksumMismatch {
            file: "insider-2024.parquet".into(),
            expected: "aaa".into(),
            actual: "bbb".into(),
        };
        let s = e.to_string();
        assert!(s.contains("aaa") && s.contains("bbb") && s.contains("insider-2024.parquet"));
    }
}
